...
#I'm storing each csv as an object
...
```{r}
#The buyer by year numbers are higher than the buyer totals because it separates the transactions by year, meaning there are multiple entries for most of the buyers, save those who only bought opioids in one year.

#the buyer addresses might be all the pharmacies with a DEA buyer number, but many of these may not have purchased opioids
```
## Analysis
...
```{r}
VA_find <- left_join(buyer_totals, buyer_addresses, by = "buyer_dea_no") %>%
arrange(desc(total_pills))

#the VA consolidated mail outpatient pharmacy has like a half a billion pilles sent to it. It's in North Charleston, SC, at 3725 rivers avenue, according to https://www.pbm.va.gov/PBM/CMOP/VA_Mail_Order_Pharmacy.asp the VA operates a mail order pharmacy that ships people pilles, it seems the North Charleston thing is where these prescriptions are consolidated before mailing. 
```
...
```{r}
maryland_buyers <- left_join(buyer_annual_by_year, buyer_addresses, by = "buyer_dea_no") %>%
  group_by(buyer_state.x) %>%
  filter(buyer_state.x == "MD") %>%
  filter(buyer_bus_act.x == "PRACTITIONER")%>%
  filter(year == "2010") %>%
  arrange(desc(dosage_unit))

#Rakesh Mathur's license was suspended in February 2012 for improper prescibing practices according to the Maryland State Physician Board. https://www.mbp.state.md.us/bpqapp/Profile.aspx#DA he ordered over 1/3 of a million opioid pills in 2012
```
...
#West Virginia with 464.88 pills per person. I've learned to break these down into the most fundamental parts before doing stuff like joins. It's probably best to write this as a statement in formal logic before I write it in R so I know which constituent elements I have
