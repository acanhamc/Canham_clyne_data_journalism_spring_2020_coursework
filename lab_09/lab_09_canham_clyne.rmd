...
#the doctors who overprescribed Oxy are effectively hitmen imo
```
...
pills_population_income %>%
 ...
 group_by(quintile) %>%
 summarise(media_household_income = mean(median_household_income),
            average_pills_per_person = mean(average_pills_per_person))  
...
ggplot(pills_population_income) +
  ...
  scale_x_continuous(labels = comma)  +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  labs(x="Average Annual Pills Per Person", y="Median Household income", title="", caption = "Source: DEA ARCOS database, via Washington Post, Census Data")
...
correlation_money_pills <- pills_population_income %>%
  ungroup() %>%
  select(average_pills_per_person, median_household_income) 
# And calculate significance

cor.test(correlation_money_pills$median_household_income, correlation_money_pills$average_pills_per_person)

#This shows that there is a slight negative correation between average pills per person and median household income. This is sort of what we'd expect. The P-value is very low, it says it's the same as with the death rate above. IDK what that means, but it seems to suggest this correlation is statistically significant. 

#on the whole, this analysis suggests there's a relationship between income and the number of pills shipped to a county, but that this relationship may be weaker than the one between death and pills per person
```
