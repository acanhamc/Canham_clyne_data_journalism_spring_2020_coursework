...
```{r}
md_pills_year <- arcos_county_pills_per_year %>%
  filter(buyer_state == "MD") %>%
  select(buyer_county, year, dosage_unit)

options(scipen=999)

ggplot(md_pills_year) +
geom_bar(stat="identity", aes(year, dosage_unit), fill='red') +
labs(x="Year", y="Total pills", title = "Maryland Opioid Doses by year and county", caption = "Source: DEA ARCOS database, via Washington Post") +
  scale_x_continuous(breaks = c(2006, 2007, 2008, 2009, 2010, 2011, 2012, 2013, 2014)) +
  scale_y_continuous(labels = comma)

#unlike baltimore, where pills declined from 2006 to 2008, and then plateaued in 2012, Maryland pill totals rose steadily to 2011 and then began to fall gradually
```
...
```{r}
us_pop_2012_by_state <- state_population(key = key) %>%
  clean_names() %>%
  filter(year == 2012)
```
...
```{r}
population_dosage <- us_pop_2012_by_state %>%
  inner_join(country_pills_2012, by= ('buyer_state'))
```

```{r}
ggplot(population_dosage) +
  geom_point(aes(population, dosage_unit)) +
  labs( x="2012 population", y="2012 opioid pills", title="a scatterplot of US counties by population and pills received", caption = "Source DEA ARCOS database, via Washington Post", fill="buyer_state") +
  scale_y_continuous(labels = comma) +
  ...
  #geom_text_repel(aes(population, dosage_unit, label=buyer_state), 
                  #subset(population_dosage, population > 10,000,000))
#I can't get these fellas to work
```
