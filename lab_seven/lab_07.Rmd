...
```{r}
# Comment The counties listed in Pills but not in Population all seem to be counties or municipalities in territories, and a couple in Alaska. At the end, there are some unnamed areas with no listed buyer counties in CT, CA and NV, and a couple other spots. Some received over 200,000 pills. Basically, the fear here is that some specific buyers or locales could be overlooked, producing a distorted view of county and state totals, and potentially obscuring the role of specific pharmacies. 
```
...
```{r}
# Define API Key
census_api_key = "05682c8a70828581d78d29e91ba8af8f0a83c218"
...
county_geodata <- get_acs(geography = "county", key = census_api_key,
              variables = "B01001_001", geometry = TRUE)

state_geodata <- get_acs(geography = "state", key = census_api_key,
              variables = "B01001_001", geometry = TRUE)
...
county_geodata_shifted <- get_acs(geography = "county", key = census_api_key,
              variables = "B01001_001", geometry = TRUE, shift_geo = TRUE)

state_geodata_shifted <- get_acs(geography = "state", key = census_api_key,
              variables = "B01001_001", geometry = TRUE, shift_geo = TRUE)
...
geo_pills_population_state %>%
  ...
  geom_sf(lwd = 0) +
  theme_map() +
  labs(fill='Pills per Person',title="Opioid Doses per Person was hightest in Appalachia between 2006 and 2014", subtitle = "Most regions saw increases in per capita shipments over the years 2006-2014, particularly the south and the west coast", caption = "Source: ARCOS database analysis and Census geographic data") +
  theme(legend.position="bottom") +
  scale_fill_viridis_c(option = "magma",labels = comma)
...
```{r}
pills_population_county_arcos <- arcos_county_population_per_year %>%
  left_join(arcos_county_pills_per_year, by = c("countyfips", "year", "buyer_county","buyer_state"))
```

```{r}
pills_population_county_2009 <- pills_population_county_arcos %>%
   mutate(statefips = str_sub(countyfips, 1,2)) %>%
   select(statefips, countyfips, buyer_county, buyer_state,year, population,dosage_unit)
```

```{r}
pills_population_county_2009 %>%
  filter(year==2009) %>%
  group_by(statefips, countyfips, buyer_state, buyer_county) %>%
  summarise(total_pills=sum(dosage_unit, na.rm = TRUE),
            total_population=sum(population, na.rm=TRUE)) %>%
  mutate(pills_per_person=total_pills/total_population)
...
# getting an error that goes "Error in mapview(geo_pills_population_county_2009, zcol = "estimate",  : 
...
```{r}
md_counties <- county_geodata_shifted %>%
 inner_join(md_counties, by=c("GEOID" = "countyfips"))
```
...
md_counties %>%
  ...
  geom_sf(lwd = 0) +
  theme_map() +
  labs(fill='Pills per Person', title="Per Capita opioid doses peaked in Maryland in the early 2010s", subtitle = "PEr capita does were highest in the western counties and in the northeastern counties, but lower in the larger, central couties, and in souther Maryland", caption = "Source: ARCOS database analysis and Census geographic data") +
  theme(legend.position="bottom") +
  scale_fill_viridis_c(option = "magma",labels = comma)
