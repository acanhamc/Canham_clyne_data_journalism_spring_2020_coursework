...
per_cap_pills <- arcos_county_pills_per_year %>%
  inner_join(arcos_county_population_per_year, by=c("countyfips", "year", "buyer_county","buyer_state"))
# Step 4: Filter the pills population table just for West Virginia and create the pills per person metric.
WV_map_pills <- per_cap_pills %>%
...
WV_map_prep <- county_geodata %>%
  inner_join(WV_map_pills, by=c("GEOID" = "countyfips"))
# Step 6: Draw the facet map
WV_map_prep %>%
  ggplot(aes(fill = pills_per_person)) +
  geom_sf(lwd = 0) +
  theme_map() +
  labs(fill='Pills Per Person',title="Pills per person in every WV County 2006-2014", caption = "Source: U.S. Census ACS, Washington Post ARCOS Database") +
  theme(legend.position="right") +
  scale_fill_viridis_c(option = "magma",)

#Pills per person were highest in the southwest of the state. Also, Harpers Ferry is a very nice spot.
```
...
```{r}
Kentuck_WV_pills <- per_cap_pills %>%
  filter(buyer_state %in% c('WV', 'KY')) %>%
  filter(year %in% 2006, 2007, 2008, 2009) %>%
  group_by(countyfips, buyer_county, buyer_state, year) %>%
  summarise(total_pills = sum(dosage_unit),
            total_population = sum(population)) %>%
  mutate(pills_per_person = total_pills/total_population)

Kentuck_WV_map <- county_geodata %>%
  inner_join(Kentuck_WV_pills, by=c("GEOID" = "countyfips"))
...
Kentuck_WV_map %>%
  ...
  geom_sf(lwd = 0) +
  theme_map() +
  labs(fill='Pills Per Person',title="Pills per person in every County in Kentucky and West Virginia from 2006-2009", caption = "Source: U.S. Census ACS, Washington Post ARCOS Database") +
  theme(legend.position="right") +
  scale_fill_viridis_c(option = "magma",)
