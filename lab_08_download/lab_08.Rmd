...
avg_url <- 'https://raw.githubusercontent.com/smussenden/spring20-data-journalism/master/labs/lab_05/data/county_pills_per_year.csv'

average_county_pills_per_year <- read_csv(avg_url)
...
```{r}
county_white_only <- get_acs(geography = "county", 
...
```{r}
opioid_death_scrape <- read_html('https://www.cdc.gov/drugoverdose/data/analysis.html')%>%
  html_nodes('table') %>%
  html_table(header=1, fill=TRUE) %>%
    as.data.frame
