...
ggplot (mingo_pills_per_year) +
  ...
  labs(x="Year", y="Total pills", title = "In Mingo County, opioid doses peaked in 2008, at nearly 10 million pills. They fell sharply across the next few years.", subtitle = "Opioid Pills Sent to Mingo Co. 2006-2014", caption = "Source: DEA ARCOS database, via Washington Post") +
  scale_x_continuous(breaks = c(2006, 2007, 2008, 2009, 2010, 2011, 2012, 2013, 2014)) +
  scale_y_continuous(labels = comma)
   
  ggsave("mingo.png", plot=last_plot())
  
  #So I looked at sections of the baltimore code to give me a blueprint for this.
```
**Task**: In the tutorial above, you followed the steps to create the graphic with the title: 
...
```{r}
west_virginia_2012 <- arcos_county_pills_per_year %>%
...
```{r}
wv_pop_2012 <- county_population(key = key) %>%
  clean_names() %>%
  filter(buyer_state == "WV", year=="2012") %>%
  select(buyer_county, population)
```
...
```{r}
wv_percapita_pills_count <- west_virginia_2012 %>%
  inner_join(wv_pop_2012, by=("buyer_county")) %>%
  select(buyer_county, dosage_unit, population)
  
```

```{r}
ggplot(wv_percapita_pills_count) +
  geom_point(aes(population, dosage_unit))  +
  labs(x="2012 population", y="pills received in 2012", title="Raleig, Cabell and Kanawha counties had higher numbers of opioids relative to population among the state's large counties", caption = "Source: DEA ARCOS databas, via Washington Post", fill="buyer_county") +
  scale_x_continuous(labels = comma) +
  scale_y_continuous(labels = comma) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  geom_smooth(aes(population, dosage_unit), method = "lm", se = FALSE) + 
  geom_text_repel(aes(population, dosage_unit, label=buyer_county),
                  subset(wv_percapita_pills_count, population > 75000))
```
