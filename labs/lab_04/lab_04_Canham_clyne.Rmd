...
#loading in the data using the read tsv function
...
```{r}
mingo_working <- mingo %>%
  mutate(distributor_address_complete = paste0(reporter_address1,", ",reporter_address2, " ", reporter_city, ", ", reporter_state, " ", reporter_zip))

#just modeling this on the function above, I hope it works.I don't know how to get rid of the null values.
```
...
```{r}
mingo_working <- mingo %>%
  mutate(distributor_state = case_when(
    reporter_state == 'WV' ~ "in_state_distribution",
    TRUE ~ "out_of_state_distribution"
  ))
#that double equal sign is interesting
```
...
mingo %>%
  ...
  summarise(pills_in_pipeline = sum(dosage_unit)) %>%
  arrange(desc(pills_in_pipeline))
```
...
```{r}
mingo %>%
  group_by(reporter_name, reporter_state, reporter_county, reporter_city, reporter_address1) %>%
   summarise(shipments = n(),
            total_pills = sum(dosage_unit), 
            avg_pills_per_shipment = mean(dosage_unit),
            min_pills_in_shipment = min(dosage_unit),
...
use case_when() to group by a part of a column name. You could google something like "use case_when to group by part of a name, or by word" something like that. Basically what you want to do is figure out how to group either by a specific word in a string of words, or by the first few characters in the column name. Alternatively, you could also see if there's a case_when function that allows you to select multiple categories, so that differently named columns produce the same output, something like case_when "and/or."
