...
opioid_scrape_working <- opioid_scrape %>%
  ...
  filter(row_number() != 36) %>%
  mutate(opioid_involved_overdose_deaths_100_000_persons1_2018 = na_if(opioid_involved_overdose_deaths_100_000_persons1_2018, "*")) %>%
  rename(deaths = opioid_involved_overdose_deaths_100_000_persons1_2018) %>%
  rename(rx_rate = opioid_prescriptions_100_persons2_2018) %>%
...
#I still get an N/A for this. going to mess around with it a bit. I can't really figure out what's wrong
...
mutate(medicare_rural_opioid_prescribing_rate = na_if(medicare_rural_opioid_prescribing_rate, "NO VALUE PROVIDED")) %>%
  ...
  mutate(medicare_urban_opioid_prescribing_rate = as.numeric(medicare_urban_opioid_prescribing_rate)) %>%
  mutate(medicare_rural_opioid_prescribing_rate = as.numeric(medicare_rural_opioid_prescribing_rate)) 
