...
#using the read tsv function to read the tsv
```
## Examine the Data
...
```{r}
#while I know how to do this, view broke r
#view(baltimore)
```
...
```{r}
glimpse(baltimore)
#the data has 290908 rows and 42 columns
#The reporter zip need not be a dbl value. I'm not sure what unit means, but I uh probably doesn't need to be a character.
...
#it would appear that Newcare Home Health Services at 3423 Sinclair Lane had the most pills sent to it, more than 6 millions pills!
#The DEA has indicted the owners of a Baltimore Pharmacy for illegally selling millions of hydrocodone pills in 2006, according to a DOJ press release. The state of Maryland has shut down the pharmacy, Newcare Home Health Services, formerly of Sinclair lane. The pharmacy, which received 6.1 million pills in 2006, allegedly was used to sell vicodin over the internet.
```
...
baltimore %>%
  ...
  summarise(shipped_pills=sum(dosage_unit))%>%
  arrange(desc(shipped_pills))

#DBA McKesson drug company shipped 58.267,330 pills to baltimore in the years we're looking at from multiple distribution locations. McKesson's global HQ is in Irving, Texas. 
```
...
#The Pharmacy is Professiona Pharmacy Service at the Sinai Hospital, which got 3,352,600 pills. This pharmacy is located in a hospital, so it might not be super sus that they got millions of pills. We would have to do some more reporting to figure out it that sort of thing is anomalous. 
...
#baltimore%>%
 # group_by(buyer_name)%>%
 # summarise(pills = sum(dosage_unit))%>%
  #mutate(pills_rank= percent_rank(dosage_unit))%>%
  #arrange(desc(pills))
#I was working on this at the end
```
